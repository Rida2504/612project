...
=======
    parser.add_argument(
        "--tag", type=str, default="", help="Label prefix for stdout status lines."
    )
    parser.add_argument(
        "--rerun",
        action="store_true",
...
=======
    ...
    print(
        f"[{tag}] warm: cuda+torch+gsplat imported in {time.time()-t_init:.1f}s",
        flush=True,
    )
>>>>>>> main
...
=======
        print(
            f"[{tag}] {i+1}/{len(tasks)} START prompt={prompt!r} seed={seed}",
            flush=True,
        )
        r = run_one(prompt, seed, config, skip_if_exists=not args.rerun)
        status = r["status"]
        print(
            f"[{tag}] {i+1}/{len(tasks)} END   status={status} elapsed={r['elapsed']}s "
            f"splat={r.get('splat')} err={r.get('error')}",
            flush=True,
        )
        if status == "ok":
            ok += 1
    print(
        f"[{tag}] DONE ok={ok}/{len(tasks)} total={time.time()-t_total0:.1f}s",
        flush=True,
    )
>>>>>>> main
